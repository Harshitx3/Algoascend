...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const fullMobile = isEmail(formData.mobileNumber) ? formData.mobileNumber : getFullMobileNumber();

    try {
      
      const users = JSON.parse(localStorage.getItem('users') || '[]');
      const user = users.find(u => u.mobile === fullMobile || u.email === formData.mobileNumber);
      ...
      const compareResponse = await axios.post('http://127.0.0.1:5000/api/auth/compare-password', {
      ...
      const tokenResponse = await axios.post('http://127.0.0.1:5000/api/auth/generate-token', {
        id: user.id,
        email: user.email,
        mobile: user.mobile
      });
      ...
      <div className="w-full max-w-md bg-zinc-100 rounded-3xl shadow-2xl overflow-hidden border border-zinc-800/50">
        <div className="p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 text-white font-bold py-4.5 px-4 rounded-2xl hover:bg-blue-700 transition duration-300 shadow-xl shadow-blue-600/30 mt-8 text-lg"
            >
              Log In
            </button>
          </form>
