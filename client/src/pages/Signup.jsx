...
const Signup = () => {
  ...
  const checkUserExists = () => {
    ...
    if (existingUsers.find(u => u.mobile === fullMobile)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const hashResponse = await axios.post('http://127.0.0.1:5000/api/auth/hash-password', {
        password: formData.password
      });
      const { hashedPassword } = hashResponse.data;
